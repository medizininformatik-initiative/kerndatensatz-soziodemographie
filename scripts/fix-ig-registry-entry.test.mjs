...
const packageId = "de.medizininformatikinitiative.kerndatensatz.base";
const canonical =
...
const request = {
  ...
  category: "Research",
  "ci-build": `${publicationBase}/branches/develop/`,
  "registry-description":
    "Implementation Guide for the Medical Informatics Initiative Core Dataset " +
    "Base modules Person, Diagnosis, Procedure, and Treatment Case.",
  "registry-country": "de",
...
test("expects the Publisher's ballot edition name for ballot publications", () => {
  const ballotEntry = targetEntry({ history, language: ["en", "de"] });
  ballotEntry.editions[0].name = `${request.sequence} Ballot`;
  const files = fixture(ballotEntry);
  writeFileSync(
    files.requestFile,
  ...
  const plainEntry = fixture(targetEntry({ history, language: ["en", "de"] }));
  writeFileSync(
    plainEntry.requestFile,
    JSON.stringify({ ...request, status: "ballot" }),
  );
  assert.throws(
    () =>
